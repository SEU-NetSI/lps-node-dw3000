...
static uint8_t rx_queue_storage[RX_QUEUE_SIZE * RX_QUEUE_ITEM_SIZE];

/* rx buffer used in rx_callback */
static uint8_t rx_buffer[FRAME_LEN_MAX];

void queueInit() {
  rx_queue = xQueueCreateStatic(RX_QUEUE_SIZE, RX_QUEUE_ITEM_SIZE, rx_queue_storage,
                                &rx_queue_buffer);
}

SPI_HandleTypeDef hspi1;
uint8_t alignedBuffer[FRAME_LEN_MAX + 1];
