...
void queueInit() {
  txQueue = xQueueCreateStatic(TX_QUEUE_SIZE, TX_ITEM_SIZE, txQueueStorage,
                               &txQueueBuffer);
  rxQueue = xQueueCreateStatic(RX_QUEUE_SIZE, RX_ITEM_SIZE, rxQueueStorage,
                               &rxQueueBuffer);
}
...
static void uwbRxTask(void *parameters) {
  ...
  while (true) {
    if (xQueueReceive(rxQueue, &packetCache, portMAX_DELAY)) {
        printf("uwbRxTask:  Rx Sequence Number = %d\r\n", packetCache.header.message_sequence);
    }
  }
}
...
static void uwbRangingTask(void *parameters) {
  ...
  while (true) {
    Ranging_Message_t packetCache;
    packetCache.header.message_sequence = seq_number;
    seq_number++;
    printf("uwbRxTask:  Tx Sequence Number = %d\r\n", packetCache.header.message_sequence);
...
void rx_cb() {
  ...
  if (dataLength != 0 && dataLength <= FRAME_LEN_MAX) {
    dwt_readrxdata(rxBuffer, dataLength - FCS_LEN,
